...
class Keyboard extends events.EventEmitter{
	constructor(container){
		...
		//the midi input
		this._midi = new Midi()
		this._midi.on('keyDown', (note, velocity) => {
			this.keyDown(note)
			this._emitKeyDown(note, velocity)
		})
	...
	_emitKeyDown(note, velocity){
		if (this._active){
			this.emit('keyDown', note, velocity)
		}
	}
	...
	keyDown(note, time=Tone.now(), ai=false, instrument = 0, velocity = 100){
		if (!this._active){
			return
		}
		if (ai) {
			this._midi.keyDown(note, time, instrument, velocity)
		}
	...
	keyUp(note, time=Tone.now(), ai=false, instrument = 0){
		...
		if (ai) {
			this._midi.keyUp(note, time, instrument)
		}
