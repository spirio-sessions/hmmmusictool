...
class Sampler{
	constructor(baseUrl='', range=[21, 108], customNotes = null){
		let slicedNotes = null;

		if (customNotes) {
			slicedNotes = customNotes;
		} else {
			...
			slicedNotes = notes.slice(lowerIndex, upperIndex)
		}
	...
	keyUp(note, time){
		if (this._loaded){
			try {
				...
				console.log('failed to stop note: ' + note)
			}
		}
	}
